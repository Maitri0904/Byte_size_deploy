...
// Supabase connection - Your exact keys from Dashboard
const SUPABASE_URL = 'https://xtgxpxekdiocjrvarkkz.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Inh0Z3hweGVrZGlvY2pydmFya2t6Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjE1NDMyNjAsImV4cCI6MjA3NzExOTI2MH0.VcqvHfF5X0S0rJtfsvrHdjAm7S7c5GUrlFhnYGgHfPs';

// Create the client (loads from the CDN script in index.html)
...
async function handleSignup(event) {
    ...
    // Real signup with Supabase (auto-hashes password)
    ...
    // Save extra info to custom 'users' table
    const { error: insertError } = await supabaseClient
        .from('users')
        .insert([{
            name: userData.name,
            email: userData.email,
            password_hash: data.user?.id || 'temp',  // Use auth ID as temp hash
            plan: 'none'
        }]);
...
async function handleLogin(event) {
    ...
    currentUser = {
        id: data.user.id,
        name: data.user.user_metadata?.name || userRow?.name || 'User',
        email: loginData.email,
        plan: userRow?.plan || 'none'
    };
...
async function selectPlan(plan) {
    ...
    const { error } = await supabaseClient
        .from('users')
        .update({ plan: plan })
        .eq('email', currentUser.email);  // Use email for simplicity
